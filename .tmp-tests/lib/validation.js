...
exports.validateRentalRequestPayload = validateRentalRequestPayload;
...
exports.validateRenterProfilePayload = validateRenterProfilePayload;
...
function validateAgencyProfilePayload(payload) {
...
function validateAgencyTripPayload(payload, partial = false) {
    ...
    const renterPartnerIds = Array.isArray(payload.renterPartnerIds)
        ? payload.renterPartnerIds.filter((value) => isValidObjectId(value))
        : [];
    const trustedRenterPartnerIds = Array.isArray(payload.trustedRenterPartnerIds)
        ? payload.trustedRenterPartnerIds.filter((value) => isValidObjectId(value))
        : [];
    const recommendedRenterPartnerIds = Array.isArray(payload.recommendedRenterPartnerIds)
        ? payload.recommendedRenterPartnerIds.filter((value) => isValidObjectId(value))
        : [];
...
function validateAgencyRenterLinksPayload(payload) {
    const linkedRenterPartnerIds = Array.isArray(payload.linkedRenterPartnerIds)
        ? payload.linkedRenterPartnerIds.filter((value) => isValidObjectId(value))
        : [];
    const trustedRenterPartnerIds = Array.isArray(payload.trustedRenterPartnerIds)
        ? payload.trustedRenterPartnerIds.filter((value) => isValidObjectId(value))
        : [];
...
function validateRentalItemPayload(payload) {
    const title = trimText(payload.title);
    const category = trimText(payload.category);
    const location = trimText(payload.location);
    ...
    const availabilityStatus = payload.availabilityStatus === "limited" || payload.availabilityStatus === "unavailable"
...
function validateRentalRequestPayload(payload) {
...
function validateTravelPostPayload(payload) {
    ...
    const gender = rawGender === "female" || rawGender === "أنثى"
        ? "female"
...
function validateAgencyVerificationStatus(value) {
    return typeof value === "string" && value in trust_1.agencyVerificationLabels.fr;
}
...
function validateRenterProfilePayload(payload) {
    const name = trimText(payload.name);
    const city = trimText(payload.city);
    const description = trimText(payload.description);
    const phone = normalizePhoneNumber(payload.phone);
