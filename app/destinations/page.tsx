...
export default async function DestinationsPage({ searchParams }: { searchParams: Promise<{ lang?: string; q?: string }> }) {
  ...
  let all: any[] = [];
  try { const result = await getPublishedDestinations(); all = Array.isArray(result) ? result : []; } catch (error) { console.error("[destinations] Failed to load published destinations.", error); }
  const destinations = all.filter((item) => {
    if (!item || !item.slug || !isValidImageUrl(item.coverImage)) return false;
    if (!q) return true;
    return [item.name, item.location, item.shortDescription].some((field) => localizeField(field, locale).toLowerCase().includes(q));
  });
  const featured = destinations.filter((item) => item.featured);
  const rest = destinations.filter((item) => !item.featured);
  const affiliateWidgets = await getPublishedAffiliateWidgets("destinations");
  const labels = {
  ...
  const card = (item: any, featuredCard = false) => (
    <Link href={withLocale(`/destinations/${item.slug}`, locale)} className="group relative block h-[430px] overflow-hidden rounded-[1.75rem] bg-forest sm:h-[480px]">
      ...
      <div className="absolute inset-x-0 bottom-0 p-5 text-white sm:p-6"><p className="text-xs font-bold uppercase tracking-[.16em] text-white/65">{localizeField(item.location, locale)}</p><h3 className={`${featuredCard ? "text-4xl" : "text-3xl"} mt-2 font-black tracking-tight`}>{localizeField(item.name, locale)}</h3>{localizeField(item.shortDescription, locale) ? <p className="mt-3 line-clamp-2 max-w-xl text-sm leading-6 text-white/75">{localizeField(item.shortDescription, locale)}</p> : null}<span className="mt-4 inline-flex items-center gap-2 text-sm font-black">{labels.explore}<ArrowUpRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span></div>
    </Link>
  );
  return <main dir={getDirection(locale)} className="page-shell max-w-[1440px] space-y-14 pb-24 sm:space-y-20">
    <section className="relative min-h-[540px] overflow-hidden rounded-[2.5rem] bg-forest">{destinations[0]?.coverImage ? <Image src={destinations[0].coverImage} alt={localizeField(destinations[0].name, locale)} fill priority sizes="100vw" className="object-cover" /> : null}<div className="absolute inset-0 bg-gradient-to-r from-black/85 via-black/45 to-black/10" /><div className="relative flex min-h-[540px] items-end px-6 py-10 sm:px-10 sm:py-14 lg:px-16 lg:py-16"><div className="max-w-3xl text-white"><p className="text-xs font-black uppercase tracking-[.28em] text-white/65">{labels.kicker}</p><h1 className="mt-5 text-5xl font-black leading-[.98] tracking-[-.045em] sm:text-7xl">{labels.title}</h1><p className="mt-6 max-w-2xl text-base leading-8 text-white/78 sm:text-lg">{labels.intro}</p><form className="mt-8 flex max-w-2xl gap-2 rounded-[1.25rem] bg-white p-2 shadow-2xl"><input type="hidden" name="lang" value={locale} /><Search className="my-auto ml-3 h-5 w-5 shrink-0 text-ink/45" aria-hidden="true" /><input name="q" defaultValue={params.q || ""} placeholder={labels.search} aria-label={labels.search} className="min-w-0 flex-1 bg-transparent px-2 py-3 text-sm text-ink outline-none" /><button className="rounded-[.9rem] bg-forest px-5 py-3 text-sm font-black text-white transition hover:bg-clay">{labels.submit}</button></form></div></div></section>
    ...
    <AffiliateWidgetSection widgets={affiliateWidgets} title={locale === "ar" ? "عروض السفر والحجوزات" : locale === "fr" ? "Expériences et offres de voyage" : "Travel experiences and partner offers"} />
