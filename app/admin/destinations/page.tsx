...
import DestinationAdminActions from "@/components/destination-admin-actions";
...
export default async function AdminDestinationsPage({ searchParams }: { searchParams: Promise<{ lang?: string }> }) {
  ...
  return <main dir={locale === "ar" ? "rtl" : "ltr"} className="page-shell space-y-8 pb-20">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div><p className="text-xs font-black uppercase tracking-[.25em] text-clay">Content</p><h1 className="mt-2 text-4xl font-black">Destinations</h1><p className="mt-2 text-sm text-ink/55">Manage publishing, visibility and destination content.</p></div>
    ...
    <div className="overflow-hidden rounded-[2rem] border border-ink/10 bg-white shadow-card">
      <div className="hidden grid-cols-[96px_minmax(0,1fr)_110px_100px_180px] gap-4 border-b border-ink/10 bg-sand/30 px-5 py-3 text-xs font-black uppercase tracking-wider text-ink/45 md:grid"><span>Image</span><span>Destination</span><span>Status</span><span>Featured</span><span>Actions</span></div>
      <div className="divide-y divide-ink/10">
        {destinations.map((item) => <div key={item._id} className="grid gap-4 p-5 md:grid-cols-[96px_minmax(0,1fr)_110px_100px_180px] md:items-center">
          ...
          <span className={`w-fit rounded-full px-3 py-1 text-xs font-black ${item.published ? "bg-forest/10 text-forest" : "bg-ink/5 text-ink/50"}`}>{item.published ? "● Published" : "● Hidden"}</span>
          <span className="text-sm font-bold">{item.featured ? "Yes" : "No"}</span>
          <DestinationAdminActions id={item._id} published={Boolean(item.published)} />
        </div>)}
        {!destinations.length && <div className="p-12 text-center text-ink/50">No destinations yet. Create the first destination to make it available for public discovery.</div>}
      </div>
    </div>
  </main>;
}
