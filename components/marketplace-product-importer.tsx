...
for(const [name,values] of domGroups){const enabled=[...new Set(values.filter(x=>!x.disabled).map(x=>x.value))];if(!enabled.length)continue;const missing=variants.length>0&&variants.every(v=>!Object.keys(v.options).some(k=>k.toLowerCase()===name.toLowerCase()));if(!missing)continue;if(variants.length===1){const copy=variants.splice(0,1)[0];enabled.forEach(value=>addVariant({...copy.options,[name]:value},{price:copy.price,stockStatus:copy.stockStatus,stockQuantity:copy.stockQuantity,sku:copy.sku,sourceVariantId:copy.sourceVariantId?copy.sourceVariantId+":"+name+":"+value:name+":"+value}));}else{const existingNames=variantOptionNames().filter(k=>k.toLowerCase()!==name.toLowerCase());const common=existingNames.length>0&&existingNames.every(k=>new Set(variants.map(v=>v.options[k]||"")).size===1);if(common){const current=variants.splice(0,variants.length);current.forEach(copy=>enabled.forEach(value=>addVariant({...copy.options,[name]:value},{price:copy.price,stockStatus:copy.stockStatus,stockQuantity:copy.stockQuantity,sku:copy.sku,sourceVariantId:copy.sourceVariantId?copy.sourceVariantId+":"+name+":"+value:name+":"+value})));}}}
