...
export function HomeHero({ slides, locale, fallback }: { slides: any[]; locale: SiteLocale; fallback: any }) {
  ...
  return <section className="overflow-hidden rounded-[2.25rem] bg-[#0f3d2e] shadow-[0_35px_100px_rgba(15,61,46,0.28)] sm:rounded-[3rem]" aria-roledescription="carousel" aria-label="Moroccan Trip featured experiences">
    <div className="relative min-h-[600px] sm:min-h-[680px] xl:min-h-[720px]">
      <Image key={slide._id || active} src={slide.image} alt={title || "Moroccan Trip"} fill priority={active === 0} sizes="100vw" className="absolute inset-0 object-cover object-center transition-opacity duration-700" />
      ...
      <div className="relative flex min-h-[600px] items-end px-5 py-8 sm:min-h-[680px] sm:px-10 sm:py-12 lg:px-14 lg:py-16 xl:min-h-[720px]">
        <div className="max-w-4xl text-white">
          ...
          {ctaLabel && <Link href={ctaUrl.startsWith("/") ? withLocale(ctaUrl, locale) : ctaUrl} className="mt-7 inline-flex rounded-full bg-[#f97316] px-7 py-4 text-sm font-black text-white shadow-[0_18px_35px_rgba(249,115,22,.3)] transition hover:-translate-y-0.5 hover:bg-white hover:text-[#0f3d2e]">{ctaLabel}</Link>}
        </div>
