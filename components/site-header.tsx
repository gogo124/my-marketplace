...
function getNavLinkClass(isActive: boolean, emphasize = false) {
  if (isActive) {
    return "rounded-full border border-emerald-900 bg-emerald-900 px-4 py-2 text-center text-white shadow-card";
  }
...
export function SiteHeader({ session }: SiteHeaderProps) {
  ...
  const primaryNavigation = (session?.user
    ...
    ...item,
    label: resolveNavItemLabel(item, locale)
  }));
  ...
  return (
    <header className="sticky top-0 z-40">
      <div className="border-b border-white/10 bg-[#0b2b22] px-4 py-2 text-center text-[11px] font-semibold tracking-[0.08em] text-white/90 sm:px-6 sm:text-xs">
          ...
          <div className="flex min-w-0 items-center justify-between gap-3">
            <div className="flex min-w-0 items-center gap-2 sm:gap-3">
              <Link
                ...
                className="flex max-w-full items-center rounded-[1.4rem] border border-white/70 bg-white/90 px-2 py-1.5 shadow-[0_16px_35px_rgba(15,61,46,0.1)] sm:px-3 sm:py-2"
              >
              ...
              <div className="hidden items-center gap-1 rounded-full border border-forest/10 bg-white/80 p-1 text-xs font-bold shadow-[0_12px_30px_rgba(15,61,46,0.08)] sm:flex">
                <Link
                  ...
                  className={`rounded-full px-3 py-1.5 ${locale === "ar" ? "bg-forest text-white" : "text-ink/60 hover:bg-sand/80"}`}
                >
                  AR
                </Link>
                <Link
                  href={buildLink(pathname, "fr", searchParams)}
                  className={`rounded-full px-3 py-1.5 ${locale === "fr" ? "bg-forest text-white" : "text-ink/60 hover:bg-sand/80"}`}
                >
                  FR
                </Link>
                <Link
                  href={buildLink(pathname, "en", searchParams)}
                  className={`rounded-full px-3 py-1.5 ${locale === "en" ? "bg-forest text-white" : "text-ink/60 hover:bg-sand/80"}`}
                >
                  EN
                </Link>
              </div>
            ...
            <button
              ...
              className="shrink-0 rounded-full border border-forest/10 bg-white px-3 py-2 text-sm font-semibold text-ink shadow-[0_12px_30px_rgba(15,61,46,0.08)] xl:hidden"
            >
              {isOpen ? marketing.close : marketing.menu}
            </button>
          </div>
            ...
            <div className="flex min-w-0 flex-col gap-2 xl:flex-row xl:flex-wrap xl:items-center xl:justify-end">
              ...
              {session?.user ? (
                <>
                  ...
                  {utilityNavigation.length > 0 ? (
                    <div className="flex flex-col gap-2 xl:flex-row xl:flex-wrap">
                      {utilityNavigation.map((item) => (
                        <Link key={item.href} href={item.href} className="w-full rounded-full border border-emerald-100 bg-white/80 px-4 py-2 text-center text-emerald-950 shadow-card xl:w-auto">
                          <span className="text-inherit">{item.label}</span>
                        </Link>
                      ))}
                    </div>
                  ...
                  {canCreateAgency ? (
                    <Link href={withLocale("/agency/profile", locale)} className="w-full rounded-full border border-emerald-900 bg-emerald-900 px-4 py-2 text-center text-white shadow-card xl:w-auto">
                      {copy.becomeAgency}
                    </Link>
                  ) : null}
                  {canCreateRenter ? (
                    <Link href={withLocale("/renter/profile", locale)} className="w-full rounded-full border border-emerald-100 bg-white/80 px-4 py-2 text-center text-emerald-950 shadow-card xl:w-auto">
                ...
                <>
                  <Link href={withLocale("/login", locale)} className={`${getNavLinkClass(pathname === "/login")} w-full xl:w-auto`}>
                    <span className="text-inherit">{copy.login}</span>
                  </Link>
                  <Link href={withLocale("/register", locale)} className="w-full rounded-full border border-emerald-900 bg-emerald-900 px-4 py-2 text-center font-semibold text-white shadow-[0_16px_35px_rgba(249,115,22,0.28)] xl:w-auto">
                    <span className="text-inherit">{copy.register}</span>
                  </Link>
                </>
              )}
            </div>
